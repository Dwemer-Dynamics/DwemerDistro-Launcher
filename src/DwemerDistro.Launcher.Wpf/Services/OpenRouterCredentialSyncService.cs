...
def ensure_api_badge_table(db):
    create_result = psql(db, """
CREATE TABLE IF NOT EXISTS public.core_api_badge (
    id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    label text NOT NULL UNIQUE,
    api_key text NOT NULL DEFAULT ''
);
CREATE INDEX IF NOT EXISTS idx_core_api_badge_label_lower ON public.core_api_badge (LOWER(label));
""")
    if create_result.returncode != 0:
        return False, (create_result.stderr or create_result.stdout).strip()

    for label in DEFAULT_API_BADGES:
        insert_result = psql(db, """
INSERT INTO public.core_api_badge(label, api_key)
SELECT __LABEL__, ''
WHERE NOT EXISTS (
    SELECT 1
    FROM public.core_api_badge
    WHERE LOWER(label) = LOWER(__LABEL__)
);
""".replace("__LABEL__", sql_literal(label)))
        if insert_result.returncode != 0:
            return False, (insert_result.stderr or insert_result.stdout).strip()
...
for item in TARGETS:
    ...
    if error:
        ...
        continue
    if not exists:
        created, create_error = ensure_api_badge_table(db)
        if not created:
            statuses.append({
                "targetName": target_name,
                "databaseName": db,
                ...
                "updated": False,
                "skipped": False,
                "statusText": "API key table setup failed",
                "error": create_error,
            })
            continue
